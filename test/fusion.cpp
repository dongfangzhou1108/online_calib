...
int main(){
    ...
    for(int i=0;i<contoursPts.size();i++) { 
		if(contoursPts[i].size()<contoursLen)
			continue;
        cv::drawContours(contoursEdge,contoursPts,i,cv::Scalar((uchar)contoursPts[i].size()),2,8,hierarchy);
    } 
	...
	cv::Mat output;
    cv::cvtColor(contoursEdge, output, CV_GRAY2BGR);
	...
	for(auto&pts:projPts){
		...
		cv::circle(output, pts, 2,cv::Scalar(b,g,r),1);
	}

    cv::imshow("output",output); 
    cv::waitKey();
    cv::destroyAllWindows();
	cv::imwrite("/home/dfz/dfz/master/Code/calibCameraLiDAR/output/edgeProj.png",output);
	return 0;
}